...
def main(credentials):
    ...
    session.execute(
        ...
        + "language"
        + " (Language TEXT, TrumpPositive INT, TrumpNeutral INT, TrumpNegative INT, BidenPositive INT, BidenNeutral INT, BidenNegative INT, Total INT, PRIMARY KEY(Language));"
    )
    session.shutdown()
    # main logic starts here
    data = spark.read.csv("language-version", header=False, schema=clean_schema)
    data = data[data["language"] != "Unknown"]
    data = data.groupBy("language").agg(
        functions.sum(
            functions.when(
